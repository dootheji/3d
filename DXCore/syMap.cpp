...
bool syMap::UpdateBuffer()
{
	...
	m_vTangentList.resize(m_dxObj.m_iNumVertex);
...
void syMap::InitFaceNormals()
{
	...
	{
		m_FaceNormals[iFace] = D3DXVECTOR3(0, 1, 0);
	}
}
	...
	for (int iRow = 0; iRow < m_iNumCellRows; iRow++)
	{
		for (int iCol = 0; iCol < m_iNumCellCols; iCol++)
		{
			...
			*/
			...
			m_VertexList[i0].n += vNormal;
			m_VertexList[i1].n += vNormal;	
			m_VertexList[i2].n += vNormal;
			D3DXVec3Normalize(&m_VertexList[i0].n, &m_VertexList[i0].n);
			D3DXVec3Normalize(&m_VertexList[i1].n, &m_VertexList[i1].n);
			D3DXVec3Normalize(&m_VertexList[i2].n, &m_VertexList[i2].n);
*/
...
bool    syMap::Load(ID3D11Device* pd3dDevice, syMapDesc& md)
{
	...
	m_pMapTex = I_TextureMgr.GetPtr(iIndex)->m_pSRV;
...
void syMap::PreRender(ID3D11DeviceContext* pContext, UINT iSize)
{
	...
	pContext->PSSetShaderResources(0, 1, m_pMapTex.GetAddressOf());
