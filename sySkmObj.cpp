...
HRESULT sySkmObj::CreateVertexData()
{
	...
	for (int iObj = 0; iObj < m_tMeshList.size(); iObj++)
	{
		...
		for (int iObj = 0; iObj < iNumBone; iObj++)
		{
			...
			_ftscanf_s(pStream, _T("%f %f %f %f\n\t%f %f %f %f\n\t%f %f %f %f\n\t%f %f %f %f"),
				&m_NodeTMList.g_pMatrix[iObj]._11,
				&m_NodeTMList.g_pMatrix[iObj]._12,
				&m_NodeTMList.g_pMatrix[iObj]._13,
				&m_NodeTMList.g_pMatrix[iObj]._14,

				&m_NodeTMList.g_pMatrix[iObj]._21,
				&m_NodeTMList.g_pMatrix[iObj]._22,
				&m_NodeTMList.g_pMatrix[iObj]._23,
				&m_NodeTMList.g_pMatrix[iObj]._24,

				&m_NodeTMList.g_pMatrix[iObj]._31,
				&m_NodeTMList.g_pMatrix[iObj]._32,
				&m_NodeTMList.g_pMatrix[iObj]._33,
				&m_NodeTMList.g_pMatrix[iObj]._34,

				&m_NodeTMList.g_pMatrix[iObj]._41,
				&m_NodeTMList.g_pMatrix[iObj]._42,
				&m_NodeTMList.g_pMatrix[iObj]._43,
				&m_NodeTMList.g_pMatrix[iObj]._44);
			D3DXMatrixTranspose(&m_NodeTMList.g_pMatrix[iObj], &m_NodeTMList.g_pMatrix[iObj]);
		}
...
bool sySkmObj::PreRender(ID3D11DeviceContext* pContext)
{
	...
	pContext->Map(m_cbNodeTM.Get(), 0, D3D11_MAP_WRITE_DISCARD,
		0, &mapRes);
	memcpy(mapRes.pData, &m_NodeTMList, sizeof(CB_BoneAnim));
	pContext->Unmap(m_cbNodeTM.Get(), 0);
